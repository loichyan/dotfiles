...
local colorschemes = {
    ...
    tokyonight = function(minv)
        minv.builtin.tokyonight.config.style = "strom"
...
local function custom(minv)
    ...
    require("custom.vscode").setup(minv)
    ...
    -- Match tmux theme.
    minv.builtin.ui.lualine.options.component_separators = { left = "", right = "" }
    minv.builtin.ui.lualine.options.section_separators = { left = "", right = "" }

    -- Luasnip
    minv.builtin.cmp.luasnip = {
        history = false,
    ...
    minv.builtin.ui.tree.git.ignore = false
    vim.list_extend(minv.builtin.ui.tree.filters.custom, { [[^target$]] })

    -- Cmp sources
    vim.list_extend(minv.builtin.cmp.config.sources, { { name = "nvim_lsp_signature_help" } })

    -- Autocmds
    vim.list_extend(minv.autocmds.q_to_close, { "notify" })

    -- Bufferline
    minv.builtin.ui.bufferline = vim.tbl_extend("force", minv.builtin.ui.bufferline, {
    ...
    -- Dashboard
    table.insert(
        minv.builtin.ui.dashboard.buttons,
        2,
    ...
    -- Treesitters
    --   minv.builtin.treesitter.config.ensure_installed = "all"

    -- Lsp servers
    minv.builtin.lsp.installer.ensure_installed = {}

    -- Rust
    local lsp = minv.builtin.lsp.config
    ...
    if ok_lua_dev then
        ...
        lsp.servers.sumneko_lua = lua_dev
    end
    lsp.formatters.stylua = {}

    -- Prettier
    lsp.formatters.prettier = {}
...
require("minv").setup(function(minv)
    require("custom.vscode").setup(minv)
    return {}
end)
